...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null); // Réinitialiser les erreurs avant une tentative

    try {
      await login(username, password);
      navigate("/dashboard"); // Rediriger vers le tableau de bord après connexion réussie
    } catch (err: any) {
      setError(err.message); // Afficher l'erreur en cas d'échec
    }
  };
    ...
    <div className="bg-[#37474f] h-screen w-full flex items-center justify-center">
      <div className="bg-[#263238] p-8 flex flex-col items-center rounded-xl shadow-lg border border-gray-600 text-gray-400 w-[20%] h-[30%]">
        ...
        <button
          ...
          className="w-full mt-6 py-3 bg-gradient-to-r from-blue-400 to-blue-600 text-white font-bold rounded-lg transition-transform duration-200 hover:scale-105"
        >
          SE CONNECTER
        </button>
      </div>
